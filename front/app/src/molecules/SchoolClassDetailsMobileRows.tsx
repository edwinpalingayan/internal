            ...
            <div className={styles.subject_category_head}>
              ...
              >
                ...
                <span className={styles.subject_category_head_availability}>
                  {typeof row.ZANSEKI_SU === "string" &&
                  !row.ZANSEKI_SU.includes("残り")
                    ? `空き${row.ZANSEKI_SU}`
                    : `${row.ZANSEKI_SU}`}
                </span>
              </div>
            ...
            <Collapse in={isOpen} timeout="auto" unmountOnExit>
              ...
              <Grid container spacing={1} columns={16} sx={{ mt: 2 }}>
                {detailConfig
                  .filter(
                    (conf) =>
                      conf.valueKey !== "CLASS_LABEL_MEI" &&
                      conf.valueKey !== "KAISAI_KO_MEI",
                  )
                  .map((conf, i) => (
                    <React.Fragment key={i}>
                      <CustomGridHead size={5}>
