...
export const WeekdayDisplay: React.FC<WeekdayDisplayProps> = ({ weekdCode }) => {
...
export const DaysDisplay: React.FC<DaysDisplayProps> = ({ days, showBreak }) => {
...
export const SchoolClassDetailsViewer: React.FC<SchoolClassDetailsViewerProps> = ({
  subjectTitle,
  rows,
  ...
  React.useEffect(() => {
    if (hasError) {
      const element = document.getElementById(`school-class-details-viewer-alert`);
      console.log('Scroll target element:', element, 'subjectTitle:', subjectTitle); // TODO: remove this log for production
      if (element) {
        const yOffset = -20;
  ...
  return (
    <div>
      <div
        className={
          hasError ? `${styles.subject_title} ${styles.subject_title_error}` : styles.subject_title
        }
      >
        {subjectTitle}
      </div>
