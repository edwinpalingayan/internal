...
import { WeekdayDisplay, DaysDisplay } from '@/molecules/SchoolClassDetailsViewer';
...
interface DefaultColumnConfig<T> extends ColumnConfig<T> {
  ...
  render?: (
    ...
    idx?: number,
    checked?: boolean,
    handle?: (idx: number, row: T) => (event: React.ChangeEvent<HTMLInputElement>) => void,
  ) => React.ReactNode;
}
...
export const defaultColumns: Array<ColumnConfig<SchoolClassDetails>> = [
  ...
  },
  {
    label: '開催校',
    value: 'KAISAI_KO_MEI',
    sx: { width: '90px !important', minWidth: '90px', maxWidth: '90px !important' },
  },
  {
    label: 'クラス',
    value: 'CLASS_MEI',
  ...
  },
  {
    ...
    sx: { width: '140px !important', minWidth: '140px', maxWidth: '140px !important' },
  },
  {
    label: '回数',
    render: (row: SchoolClassDetails) =>
  ...
  ...[1, 2, 3, 4, 5, 6].map((day): DefaultColumnConfig<SchoolClassDetails> => {
    ...
    return {
      ...
      render: (row: SchoolClassDetails) =>
        row[key]
          ? React.createElement(DaysDisplay, { days: row[key] as string, showBreak: true })
          : '',
      sx: {
        width: '50px !important',
        minWidth: '50px',
