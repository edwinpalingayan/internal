...
export function generateBearerToken(): string {
  try {
    ...
    if (!secretKey || !appPrefix) {
      throw new Error('Environment variables SECRET_KEY and APP_KEY are required');
    }
    ...
    // Base64エンコード
    const base64String = CryptoJS.enc.Base64.stringify(CryptoJS.enc.Utf8.parse(prefixedHash));
